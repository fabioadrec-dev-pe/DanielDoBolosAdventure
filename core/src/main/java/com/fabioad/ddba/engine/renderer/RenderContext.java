...
public final class RenderContext implements Disposable {
     ...
     * Retorna a geometria da area util (para overlays nas barras pretas).
     */
    public LetterboxInfo present() {
        ...
        return new LetterboxInfo(winW, winH, scale, offsetX, offsetY, drawW, drawH);
    }

    /** Inicia batch em coordenadas de tela cheia (apos present). */
    public void beginScreenOverlay(LetterboxInfo box) {
        screenCamera.setToOrtho(false, box.winW, box.winH);
        screenCamera.update();
        batch.setProjectionMatrix(screenCamera.combined);
        batch.begin();
    ...
    public void endScreenOverlay() {
        batch.end();
    }

    /** Calcula letterbox atual sem redesenhar (para hit-test no update). */
    public LetterboxInfo currentLetterbox() {
        int winW = Gdx.graphics.getWidth();
        int winH = Gdx.graphics.getHeight();
